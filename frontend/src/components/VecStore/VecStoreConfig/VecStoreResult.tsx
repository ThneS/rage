...
const VecStoreResultModal: React.FC<VecStoreResultModalProps> = ({
  ...
  // 计算所有页码
  const pages = useMemo(() => {
    if (!VecStoreResult) return [];
    const pageSet = new Set<number>();
    VecStoreResult.forEach(chunk => {
  ...
  // 当前页的分块
  const pageChunks = (VecStoreResult || []).filter(chunk => chunk.metadata?.page === selectedPage);
...
export default VecStoreResultModal;