...
import { fetchSearchConfig, processSearch } from '@/store/slices/searchSlice';
...
const SearchConfig: React.FC<SearchConfigProps> = ({
  ...
  const { config, loading, error } = useAppSelector(state => state.search);
  ...
  useEffect(() => {
    if (selectedDocument) {
      dispatch(fetchSearchConfig(selectedDocument.id));
    }
  ...
  const handleSubmit = async (values: Record<string, any>) => {
    ...
    try {
      ...
      await dispatch(processSearch(selectedDocument.id, submitConfig));
    } catch (error: unknown) {
          ...
          <Form.Item className="mt-4 mb-0">
            <Space className="w-full justify-end">
              ...
              <Button
                ...
                style={{ minWidth: 100 }}
                disabled={!selectedDocument || !SearchResult}
              >
                查看搜索
              </Button>
            </Space>
