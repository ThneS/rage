...
import { fetchGenerateConfig, processGenerate } from '@/store/slices/generateSlice';
...
const GenerateConfig: React.FC<GenerateConfigProps> = ({
  selectedDocument,
  processing,
  onViewGenerate,
  generateResult,
  ...
  const { config, loading, error } = useAppSelector(state => state.generate);
  ...
  useEffect(() => {
    if (selectedDocument) {
      dispatch(fetchGenerateConfig(selectedDocument.id));
    }
  ...
  const handleSubmit = async (values: Record<string, any>) => {
    ...
    try {
      ...
      await dispatch(processGenerate(selectedDocument.id, submitConfig));
    } catch (error: unknown) {
        ...
        >
          ...
          <Form.Item className="mt-4 mb-0">
            <Space className="w-full justify-end">
              ...
              <Button
                type="default"
                onClick={onViewGenerate}
                style={{ minWidth: 100 }}
                disabled={!selectedDocument || !generateResult}
              >
                查看生成
              </Button>
            </Space>
